...
def list_research_templates(settings: Any) -> list[dict]:
    ...
    扫描 research_template_dir 下的 *.docx；目录不存在或为空时，回退为
    research_template_path 指向的单个兜底模板（若该文件存在）。
    ...
    fallback = settings.research_template_path
    if fallback:
        fp = Path(fallback)
        if fp.is_file():
            out.append({"name": fp.name, "label": _label_from_filename(fp.name)})
    return out
...
def resolve_template_path(settings: Any, name: str | None) -> Path | None:
    ...
    只取 basename 在模板目录内查找，拒绝路径穿越；name 为空时回退兜底模板。
    解析不到返回 None。
    """
    fallback = settings.research_template_path
    ...
    # 兼容：选择项可能就是兜底模板自身。
    if fallback_path and fallback_path.is_file() and fallback_path.name == safe_name:
        return fallback_path
    return None
